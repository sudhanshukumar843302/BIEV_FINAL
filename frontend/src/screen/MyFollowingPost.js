...
export default function MyFollowingPost() {
  ...
  const loggedInUserId = localStorage.getItem("user")?JSON.parse(localStorage.getItem("user"))._id: undefined;
  ...
  function decideProfilePhoto(eachPost){
    ...
    if(eachPost.postedBy.photo && !eachPost.postedBy.photoBlocked.includes(loggedInUserId)){
        ...
        return (
          <div className="card" key={eachPost._id}>
            ...
            <div className="card-header" style={{padding:"0"}}>
              ...
              <Link to={`/profile/${eachPost.postedBy._id}`} style={{margin:"0 10px"}}>
